...
const inner = props => ({
  ...
  fn(state) {
    ...
    const nextArgs = {
      ...state,
      ...offset(-item.offsetTop - rects.reference.height / 2 - item.offsetHeight / 2 - innerOffset).fn(state)
    };
    const el = (scrollRef == null ? void 0 : scrollRef.current) || floating;
    const overflow = detectOverflow(getArgsWithCustomFloatingHeight(nextArgs, el.scrollHeight), detectOverflowOptions);
    ...
    if (overflowRef) {
      overflowRef.current = detectOverflow(getArgsWithCustomFloatingHeight({
        ...nextArgs,
        y: nextY
